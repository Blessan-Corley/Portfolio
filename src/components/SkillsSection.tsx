...
import type { SkillCardConfig } from '../types';
...
const iconMap: Record<SkillCardConfig['iconKey'], ReactElement> = {
  code: <FiCode />,
  database: <FiDatabase />,
...
const SkillsSection = () => {
  ...
  const decoratedSkills = useMemo<DecoratedSkillCard[]>(
  ...
  const particles = useMemo<BackgroundParticle[]>(() => {
    ...
    return Array.from({ length: particleCount }, (_, i) => {
      ...
      return {
        ...
        startY: 50 + (i * 60) % 400,
        color: skill?.color ?? '#00ffff',
        size: sizes[i % 3] ?? 'small',
      };
    });
  }, [decoratedSkills, windowWidth]);

  const backgroundIcons = useMemo<BackgroundIconData[]>(() => {
    ...
    return Array.from({ length: iconCount }, (_, i) => {
      ...
      return {
        ...
        icon: techIcons[i % techIcons.length] ?? <FiCode />,
        color: skill?.color ?? '#00ffff',
      };
    });
  }, [decoratedSkills, windowWidth]);

  const circuitPatterns = useMemo<CircuitPatternData[]>(() => {
