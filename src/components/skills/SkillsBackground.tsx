...
interface CircuitPatternConfig {
  delay?: number;
  startX?: number;
  startY?: number;
  color?: string;
...
interface SkillsBackgroundProps {
  particles: FloatingParticleConfig[];
  backgroundIcons: BackgroundIconConfig[];
  circuitPatterns: CircuitPatternConfig[];
}

const FloatingParticle = ({ delay = 0, startX = 0, startY = 0, color = '#00ffff', size = 'small' }: FloatingParticleConfig) => {
  const sizeMap = {
    small: 'w-1.5 h-1.5',
    medium: 'w-2 h-2',
    large: 'w-3 h-3'
  } satisfies Record<ParticleSize, string>;
...
const BackgroundIcon = ({ delay = 0, startX = 0, startY = 0, icon, color = '#00ffff' }: BackgroundIconConfig) => (
  <motion.div
...
const CircuitPattern = ({ delay = 0, startX = 0, startY = 0, color = '#00ffff' }: CircuitPatternConfig) => (
  <motion.div
...
const SkillsBackground = ({ particles, backgroundIcons, circuitPatterns }: SkillsBackgroundProps) => (
