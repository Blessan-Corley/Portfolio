...
import type { AboutHighlightIconKey } from '../types';
...
const About = () => {
  ...
  const aboutIcons: Record<AboutHighlightIconKey, LucideIcon> = {
    code2: Code2,
    boxes: Boxes,
    layers: Layers,
    cloudCog: CloudCog,
  ...
  const carouselItems: CarouselItemData[] = aboutHighlights.map((item) => {
    const Icon = aboutIcons[item.iconKey];

    return {
      ...item,
      icon: (props: Record<string, unknown>) => <Icon size={16} strokeWidth={2} color="white" {...props} />,
    };
  });
